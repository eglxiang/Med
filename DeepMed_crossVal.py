...
inputParamsConfigLocal['input_shape'] = '44, 44, 12'
...
inputParamsConfigLocal['train_set_size'] = '500000'#'700000'
...
inputParamsConfigLocal['fp_per_case'] = '0'#set to '-1' or '0' if you dont want false positive; implemented as fp per pos, NOT fp per case! Not used if phase=='discrim'
...
def Main(inputParamsConfig):
    ...
    if len(lst_name) == numFolds:
        ...
        #            tr_len_neg = len(np.where(train_label==0)[0])
                ...
                #pdb.set_trace()
                tr_len_pos,tr_len_neg, ts_len_pos,ts_len_neg = SupportFuncs_c.load_data(inputParamsLoadData,train_x,test_x)
               # train_set, train_label, test_set, test_label, val_set, val_label = SupportFuncs.load_data(data_path, int(train_set_size),
                #                                                                                      int(test_set_size),
                 #                                                                                     int(augmentationFlag),float(positive_set
        ...
        #            data_set.create_dataset('val_label', data=val_label)
            ...
            for epoch in range(int(num_epochs)):
                ...
                val_AUC, val_varAUC = SupportFuncs_c.Pred2AUC(all_val_pred, all_val_labels)
            ...
            test_AUC, test_varAUC = SupportFuncs_c.Pred2AUC(all_test_pred, all_test_labels)
