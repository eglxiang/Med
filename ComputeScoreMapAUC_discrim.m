...
%truthing method;
% NOTE: This is different from ComputeScoreMapAUC in that in discrim mode, you need to apply the CNN to the
% patches from the screening process. One way of doing this is to apply the discrim CNN as an FCN to get scores
% across the entire volume. The scoreMap can then be filtered by multiplying it by the score_map_post_proc from the
% corresponding screening model so that only the scores located on centerInds from screening model are counted and used
% in computing AUC.
%ACHTUNG: THE CONVERSION OF CENTERINDS FROM SCREENING TO LIN INDICES ASSUMES THAT SCOREMAPS OF SCREEN/DISCRIM ARE SAME SIZE!!!!
pkg load image; %this for bwlabeln
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
modelName = 'cnn_36368_20170122152812'; %used to determine where to read discrim score maps/write results to
modelNameScreening = 'cnn_36368_20170117173055'; %the screening model used to train the discrim model; for filtering scores to center points of screening model
uniqueMaskPath = '/raida/apezeshk/lung_dicom_dir/';
masterSaveFolder = fullfile('/raida/apezeshk/temp/score_map_post_proc', modelName); %results will be written in subfolders based on truthMode and runMode
...
scoreMapFolder =  fullfile('/diskStation/LIDC/36368/score_map/',modelName, runMode); %Folder to read score maps from
lungInteriorFolder = '/raida/apezeshk/LIDC_v2/037_part_to_lung' %'/raida/apezeshk/LIDC_v2/015_lung_only'
scoreMapPostProcScreen = fullfile('/raida/apezeshk/temp/score_map_post_proc', modelNameScreening, runMode, truthMode); %folder to read from
...
for jj = 1:length(files)
        ...
        currentScoreMapDataDiscrim = load(fullfile(scoreMapFolder, currentMapName));
        currentScoreMapDiscrim = currentScoreMapDataDiscrim.scoreMap;
        
        currentScoreMapPostProcDataScreen = load(fullfile(scoreMapPostProcScreen, currentMapName));
        currentCenterIndsScreen = currentScoreMapPostProcDataScreen.centerInds;
        %ACHTUNG
        %ACHTUNG: BELOW ASSUMES THAT SCOREMAPS OF SCREEN/DISCRIM ARE SAME SIZE!!!!
        %ACHTUNG
        currentLinIndsScreen = sub2ind(size(currentScoreMapDiscrim), currentCenterIndsScreen(:,1), currentCenterIndsScreen(:,2), currentCenterIndsScreen(:,3));
        %Below is so that you can extract scores at screening center points; necessary s.t. you calculate AUC, etc off of screening candidates only
        maskScreen = zeros(size(currentScoreMapDiscrim)); 
        maskScreen(currentLinIndsScreen) = 1;
        currentScoreMapDiscrim = currentScoreMapDiscrim .* maskScreen;
        
        if lungInteriorFlag == 1 %in this case, extract only lung portion of scoreMap
            currentLungInteriorMask = logical(currentScoreMapDataDiscrim.lungInteriorMask);
            %for kk = 1:size(currentLungInteriorMask,3)
            %   se = strel('square', 3);
            %   currentLungInteriorMask(:,:,kk) = imerode(currentLungInteriorMask(:,:,kk), se);
            %end
            currentScoreMapDiscrim = currentScoreMapDiscrim.*currentLungInteriorMask;
        end
        centerIndsDiscrim = find(currentScoreMapDiscrim); %these are linear indices
        [row, col, slc] = ind2sub(size(currentScoreMapDiscrim), centerIndsDiscrim);
        centerIndsDiscrim = zeros(numel(row), 3);
        centerIndsDiscrim(:,1) = row; centerIndsDiscrim(:,2) = col; centerIndsDiscrim(:,3) = slc; %conversion to subscripts
...
%        [thresholdedMask, centerInds] = GetCandidatesFromScoreMap(currentMapMaxSupp, threshScoreMask);   
        ...
        if resizeFlag == 1
                ...
                currentNegativeInd = 1:size(centerIndsDiscrim,1); %initialize; list of all row numbers for peaks in centerInds; later these will be reduced by taking out the row numbers corresponding to positive peaks
                ...
                for m = 1:length(uniqueStats)
                        ...
                        currentNoduleCheck = ismember(centerIndsDiscrim(:,2), bounds_minx:bounds_maxx) & ismember(centerIndsDiscrim(:,1), bounds_miny:bounds_maxy) & ismember(centerIndsDiscrim(:,3), bounds_minz:bounds_maxz);
...
%                 end
                ...
                noduleMask = currentScoreMapDataDiscrim.noduleMask; %this is FCN processed noduleMask, not the full-size one!
                noduleMask_lessThan3mm = currentScoreMapDataDiscrim.noduleMask_lessThan3mm; %this is FCN processed <3mm nodule mask

                [h, w, z] = size(noduleMask);

                currentNegativeInd = 1:size(centerIndsDiscrim,1); %initialize; list of all row numbers for peaks in centerInds; later these will be reduced by taking out the row numbers corresponding to positive peaks
                currentSmallNodNegativeInd = 1:size(centerIndsDiscrim,1); %initialize; list of all row numbers for peaks in centerInds; later these will be reduced by taking out the row numbers corresponding to + peaks from small nodules only (may have overlap with >3mm + peaks)
                for m = 1:length(uniqueStats)
                        current_avgPrincipalAxisLength = uniqueStats(m).avgPrincipalAxisLength;
                        if removeSmallNodFlag == 1 %whether to ignore the <3mm nodules from the master nodule mask
                                ...
                                if current_avgPrincipalAxisLength == 0
                                        ...
                                        currentNoduleCheck = ismember(centerIndsDiscrim(:,1), bounds_minrow:bounds_maxrow) & ismember(centerIndsDiscrim(:,2), bounds_mincol:bounds_maxcol) & ismember(centerIndsDiscrim(:,3), bounds_minslc:bounds_maxslc);
                                        currentNoduleMatchInd = find(currentNoduleCheck); %row indices of peak points that fall within bounds of current nodule
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if removeSmallNodFlag == 0
                currentCaseLabels = ones(1, size(centerIndsDiscrim,1)); %initialize row vector of labels for current case
                currentCaseLabels(currentNegativeInd) = 0;
                currentCaseScores = zeros(1, size(centerIndsDiscrim, 1));
                for ii = 1:length(currentCaseScores)
                        currentCaseScores(ii) = currentScoreMapDiscrim(centerIndsDiscrim(ii,1), centerIndsDiscrim(ii,2), centerIndsDiscrim(ii,3));
                end
        ...
        elseif removeSmallNodFlag== 1
                ...
                %different tag id)
                smallNodInds = intersect(currentNegativeInd, setdiff(1:size(centerIndsDiscrim,1),currentSmallNodNegativeInd));
                ...
                centerIndsDiscrim(smallNodInds, :) = [];
        ...
        disp(['Number of center peaks in current map: ' num2str(size(centerIndsDiscrim, 1))]);
        fflush(stdout);
        ...
        save('-v7', fullfile(masterSaveFolder, runMode, truthMode, currentMapName), 'centerIndsDiscrim', 'currentCaseScores', ...
        'currentCaseLabels', 'patchSize', 'truthMode','resizeFlag', 'removeSmallNodFlag', 'runMode');
end
...
save('-v7', fullfile(masterSaveFolder, runMode, truthMode,'PostProcAUC.mat'), 'scoreMapFolder','patchSize',...
        ...
        'lungInteriorFlag', 'modelNameScreening', 'modelName');
