...
pathSavedNetwork = '/home/apezeshk/Codes/DeepMed/models/cnn_36368_20170130193004.npz'
pathSavedSamples = '/home/apezeshk/Codes/DeepMed/models/cnn_36368_20170130193004_samples.npz'
...
test_pos_paths = [os.path.join(masterPatchFolder, 'pos', x) for x in test_pos_paths]
test_neg_paths = [os.path.join(masterPatchFolder, 'neg', x) for x in test_neg_paths]
test_samples,test_labels,test_len_pos,test_len_neg = SupportFuncs.mat_generate_from_path(test_pos_paths,test_neg_paths)
...
#inputParamsConfigLocal['numberOfFCUnits'] = 600 #number of units in first fully connected layer; originally 64
...
#inputParamsConfigLocal['fp_per_case'] = '350'#set to '-1' or '0' if you dont want false positive; implemented as fp per pos, NOT fp per case! Not used if phase=='discrim'
...
#inputParamsConfigLocal['fp_model_to_use'] = 'cnn_36368_20170127195107'#only used if fp_per_case>0 or phase=='discrim'; defines model (therefore subdirectory) fp's were extracted from
...
#               data_size, test_size, learning_rate, Totall_Accuracy, Test_AUC, sts, augmentationFlag, nonlinearityToUse,
...
#              'std', 'augmentationFlag', 'nonlinearityToUse', 'dropout','tr_num_pos','tr_num_neg']
#
...
#    
#    print(
#        " Learning rate: '%s' , momentum: '%s',  num_epochs: '%s'  ,batch size: '%s'  ,data_path: '%s',Train Set Size: '%s' ,Test set Size: '%s' ,Positive set Ratio '%s' , dropout: '%s', nonlinearityToUse: '%s',augmentationFlag: '%s',number of layers: '%s', pos_test_size: '%s'" % (
#        str(learning_rate), str(momentum), str(num_epochs), str(batch_size), data_path, str(train_set_size),
#        str(test_set_size), str(positive_set_ratio), str(dropout), str(nonlinearityToUse), str(augmentationFlag),
#        str(numberOfLayers), str(pos_test_size)))
#    #print(" Phase: '%s', Number of FC Units: '%s', Number of ConvFilters: '%s'" % (str(phase), str(numberOfFCUnits), str(numberOfConvFilts)))
...
#                training_filename = os.path.join('./',input_shape+'_'+str(augmentationFlag)+'_'+str(positive_set_ratio)+'.hdf5')
#            else:
#                training_filename = os.path.join('./',input_shape+'_'+str(augmentationFlag)+'_'+str(positive_set_ratio)
#                    +'_fp' + fp_model_to_use + '_' + fp_per_case +'.hdf5')
#        else:
#            if fp_per_case == '0': #make different filenames per fp_per_case s.t. original & supplemented train set are maintained
#                training_filename = os.path.join('./',input_shape+'_'+str(augmentationFlag)+'_'+str(positive_set_ratio)+'_filt.hdf5')
#            else:
#                training_filename = os.path.join('./',input_shape+'_'+str(augmentationFlag)+'_'+str(positive_set_ratio)
...
#        inputParamsLoadData['augmentationFlag'] = int(augmentationFlag)
...
#        
#        train_set, train_label, test_set, test_label, val_set, val_label, tr_len_pos,tr_len_neg = SupportFuncs.load_data(inputParamsLoadData)    	    
...
inputParamsNetwork = dict(n_layer=numberOfLayers, shape=input_shape,dropout=float(dropout), nonLinearity=nonLinearity,
                          biasInit = biasInit, filterSizeTable = filterSizeTable, 
...
#    np.savez(filenameSamples, inputs=inputs, targets=targets, err=err, acc=acc, training_loss=training_loss, test_pred=test_pred, all_test_pred=all_test_pred,all_test_labels=all_test_labels,inputParamsConfig=inputParamsConfig)
...
#               , len(train_set), len(test_set), (test_err / test_batches), (test_acc / test_batches), test_AUC[0],
#               np.sqrt(test_varAUC), augmentationFlag, nonlinearityToUse, dropout,tr_len_pos,tr_len_neg)
#
