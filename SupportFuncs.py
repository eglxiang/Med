...
def Gen_data(inputParamsLoadData):
    data_path = inputParamsLoadData['data_path']
    train_set_size = inputParamsLoadData['train_set_size']
    test_set_size = inputParamsLoadData['test_set_size']
    augmentationRegularFlag = inputParamsLoadData['augmentationRegularFlag']
...
#            neg_dir_list.append(dirc)
    ...
    all_neg_file_path = Get_file_paths(data_path, neg_test_dir_list,
                                      len(os.listdir(os.path.join(data_path, neg_test_dir_list[0]))))
    ...
    #get full paths to all available positive patches (excluding aug versions of them, so this will give count of originals)
    pos_file_path = Get_file_paths(data_path, pos_dir_list, len(os.listdir(os.path.join(data_path, pos_dir_list[0]))))#whole positive case
...
########################################################################################################
def Get_file_paths(directory,pos_neg_direc_list,total_cases):
    ...
    size_of_dir = len(list_pso_neg_subdir)
    candidate_neg=np.random.choice(size_of_dir, total_cases, replace=False)
    for i in candidate_neg:
        file_paths.append(os.path.join(os.path.join(directory,pos_neg_direc_list[0]), list_pso_neg_subdir[int(i)]))
...
##############################################################################################################
def load_data(inputParamsLoadData):
    print("Writing data...")
...
#    noduleCaseFilterParams = inputParamsLoadData['noduleCaseFilterParams']
    ...
    if phase == 'screen':
        ...
        train_pos,train_neg,test_pos,test_neg=Gen_data(inputParamsLoadData)
    elif phase == 'discrim':
