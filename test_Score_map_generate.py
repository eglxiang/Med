...
pathSavedNetwork = '/home/shamidian/DeepMedAria/models/cnn_36368_20170201103413.npz'
pathSavedSamples = '/home/shamidian/DeepMedAria/models/cnn_36368_20170201103413_samples.npz'


#test_filename = '/home/apezeshk/Codes/DeepMed/test_500_-1_LIDC.hdf5'
test_filename = '/home/shamidian/DeepMedAria/test_500_100_36368_filt.hdf5'
...
minmaxFilePath = '/home/shamidian/DeepMedAria/max_min.json' #file containing the min/max of each case; used for normalizing
...
with h5py.File(test_filename, 'r') as hf:
    ...
    filteredCaseList = hf.get('filteredCaseList') #list with elements like 'p0034'; all cases that fit filter criteria
...
#                                    b=inputParamsNetwork['biasInit'],
#                                    )
    network = lasagne.layers.dnn.Conv3DDNNLayer(lasagne.layers.dropout(network, p=dropout), num_filters=numberOfFCUnits, pad=0, filter_size=(9, 9, 4),
                                    stride=(1, 1, 1),
                                    nonlinearity=lasagne.nonlinearities.sigmoid,
                                    W=init_norm,
                                    b=inputParamsNetwork['biasInit'],
    ...
    # will do the softmax later, it is giving error if we use it here for some reason
        network = lasagne.layers.dnn.Conv3DDNNLayer(lasagne.layers.dropout(network, p=dropout), num_filters=numberOfFCUnits, pad=0, filter_size=(1, 1, 1),
                                    stride=(1, 1, 1),
                                    nonlinearity=lasagne.nonlinearities.sigmoid,
                                    W=init_norm,
                                    b=inputParamsNetwork['biasInit'],
...
if numberOfLayers == 2:    
    ...
    W6_new = np.zeros((numberOfFCUnits,numberOfConvFilts,9,9,4)).astype('float32')
    for i in range(0,param_values[6].shape[1]): #weights for each node in FC layer form the columns
        current_node_weights = param_values[6][:,i]
        W6_new[i, :, :, :, :] = np.reshape(current_node_weights, (numberOfConvFilts,9,9,4), order = 'C')
    ...
    for currentCaseName in all_cases: #e.g. p0949_20000101_s3180.npy
        ...
        if runMode=='train':
            if currentCaseName.split('_')[0] in patient_id:# if runMode='train', skip test cases
                continue
            if currentCaseName.split('_')[0] not in filteredCases: #the case should also be one that matches filter criteria
                continue
        elif runMode=='test':
        ...
        try:
            ...
            if resizeFCNFlag == 1:            
                ...
                noduleMaskFCN, noduleMaskFCN_lessThan3mm = MaskFCN.CreateNoduleMaskFCN(inputParamsConfigAll, inputParamsNoduleMaskFCN)
                ...
                lungInteriorMaskFCN = MaskFCN.CreateLungInteriorMaskFCN(inputParamsConfigAll, inputParamsLungInteriorMaskFCN)
