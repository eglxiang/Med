...
pathSavedNetwork = '/media/shamidian/sda2/ws/DeepMed/models/cnn_444412_20170717172004.npz'
pathSavedSamples = '/media/shamidian/sda2/ws/DeepMed/models/cnn_444412_20170717172004_samples.npz'


#test_filename = '/home/apezeshk/Codes/DeepMed/test_500_-1_LIDC.hdf5'
test_filename = '/media/shamidian/sda2/ws/DeepMed/test_500_100_444412_filt.hdf5'
...
score_map_path='/diskStation/LIDC/444412/score_map/' #master folder for score maps; add '/test' for test purporses if needed
...
minmaxFilePath = '/media/shamidian/sda2/ws/DeepMed/max_min.json' #file containing the min/max of each case; used for normalizing
...
#                                    b=inputParamsNetwork['biasInit'],
#                                    )
    network = lasagne.layers.dnn.Conv3DDNNLayer(lasagne.layers.dropout(network, p=dropout), num_filters=numberOfFCUnits, pad=0, filter_size=(11, 11, 6),
                                    stride=(1, 1, 1),
...
if numberOfLayers == 2:    
    W4_new = np.zeros((numberOfFCUnits,numberOfConvFilts,11,11,6)).astype('float32')
    ...
    W6_new = np.zeros((numberOfFCUnits,numberOfConvFilts,11,11,6)).astype('float32')
    for i in range(0,param_values[6].shape[1]): #weights for each node in FC layer form the columns
        current_node_weights = param_values[6][:,i]
        W6_new[i, :, :, :, :] = np.reshape(current_node_weights, (numberOfConvFilts,11,11,6), order = 'C')
